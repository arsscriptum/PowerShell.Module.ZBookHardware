...
function Write-DebugMessage {
    ...
    process {
        if ($ENV:DebugLog -eq $true) {
            # Format the message with colors if provided
            if ($PSBoundParameters.ContainsKey('BackgroundColor')) {
                Write-Host $Message -ForegroundColor $ForegroundColor -BackgroundColor $BackgroundColor 2>&1
            } else {
                Write-Host $Message -ForegroundColor $ForegroundColor 2>&1
            }
        }
    }
}
